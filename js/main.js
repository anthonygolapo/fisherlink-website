...
function fetchInformation() {
    document.getElementById("detailsModal").style.display = "flex"; // open immediately
...
function fetchSafeReport() {
    document.getElementById("safeReportModal").style.display = "flex"; // open immediately
    ws.send(JSON.stringify({
        ...
        if (response.type === "safe_report" || response.type === "sender_report") {
            ...
            if (isSenderReport) {
                tableHeader.innerHTML = `
                    <tr>
                        <th style="background-color: #0047AB; color: white;">Month</th> <!-- Add Month column for sender-wise -->
                        <th style="background-color: #0047AB; color: white;">Sender</th>
                        <th style="background-color: #b40000; color: white;">SOS Count ℹ️</th> <!-- Red for SOS -->
                        <th style="background-color: #006700; color: white;">Mark as Safe Count ℹ️</th> <!-- Green for Safe -->
                        <th style="background-color: #676767; color: white;">Not Found Count ℹ️</th> <!-- Gray for Not Found -->
                    </tr>
                `;
            } else {
                tableHeader.innerHTML = `
